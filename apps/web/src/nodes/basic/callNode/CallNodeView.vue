<template>
  <div class="node">
    <!-- 1. 表头区域：显示表名 -->
    <NodeHeader>调用{{ properties?.functionName }}</NodeHeader>
...
import type CallNodeModel from './callNodeModel';
import type { CallNodeProperties } from './callNodeModel';
import NodeField from '@/components/node/NodeField.vue';

// 1. 注入 LogicFlow 核心方法
const getNode = inject<() => CallNodeModel>('getNode');
const getGraph = inject<() => GraphModel>('getGraph');

// 2. 定义响应式数据
// 给定一个默认结构，防止渲染报错
const properties = ref<CallNodeProperties>();
...
onMounted(() => {
  ...
  const updateData = (props: CallNodeProperties) => {
    // 这里做一层合并，确保即使 props 为空也有默认值
    properties.value = { ...props };
  };

  // A. 初始化：首次加载时读取数据
  updateData(node.properties as CallNodeProperties);
  ...
  graph.eventCenter.on(
    ...
    (eventData: { id: string; properties: unknown }) => {
      ...
      updateData(eventData.properties as CallNodeProperties);
    },
  );
});
</script>
