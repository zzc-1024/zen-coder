...
import type ContinueNodeModel from './continueNodeModel';
import type { ContinueNodeProperties } from './continueNodeModel';
import NodeField from '@/components/node/NodeField.vue';

// 1. 注入 LogicFlow 核心方法
const getNode = inject<() => ContinueNodeModel>('getNode');
const getGraph = inject<() => GraphModel>('getGraph');

// 2. 定义响应式数据
// 给定一个默认结构，防止渲染报错
const properties = ref<ContinueNodeProperties>();
...
onMounted(() => {
  ...
  // A. 初始化：首次加载时读取数据
  updateData(node.properties as ContinueNodeProperties);
  ...
  graph.eventCenter.on(
    ...
    (eventData: { id: string; properties: unknown }) => {
      ...
      updateData(eventData.properties as ContinueNodeProperties);
    },
  );
});
</script>
