...
import type IndexNodeModel from './indexNodeModel';
import type { IndexNodeProperties } from './indexNodeModel';
import NodeField from '@/components/node/NodeField.vue';
import { parseType } from '@/parser/variable';

// 1. 注入 LogicFlow 核心方法
const getNode = inject<() => IndexNodeModel>('getNode');
const getGraph = inject<() => GraphModel>('getGraph');

// 2. 定义响应式数据
// 给定一个默认结构，防止渲染报错
const properties = ref<IndexNodeProperties>();
...
onMounted(() => {
  ...
  const updateData = (props: IndexNodeProperties) => {
    // 这里做一层合并，确保即使 props 为空也有默认值
    properties.value = { ...props };
  };

  // A. 初始化：首次加载时读取数据
  updateData(node.properties as IndexNodeProperties);
  ...
  graph.eventCenter.on(
    ...
    (eventData: { id: string; properties: unknown }) => {
      ...
      updateData(eventData.properties as IndexNodeProperties);
    },
  );
});
</script>
