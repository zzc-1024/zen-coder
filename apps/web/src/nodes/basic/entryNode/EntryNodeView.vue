<template>
  <div class="node">
    ...
    <!-- 2. 字段列表区域 -->
    <div class="node-body">
      <NodeField :hasInput="false" :hasOutput="true" name="流程入口" type="builtin:basic:flow" />
    </div>
  </div>
</template>
...
import type EntryNodeModel from './entryNodeModel';
import type { EntryNodeProperties } from './entryNodeModel';
import NodeField from '@/components/node/NodeField.vue';

// 1. 注入 LogicFlow 核心方法
const getNode = inject<() => EntryNodeModel>('getNode');
const getGraph = inject<() => GraphModel>('getGraph');

// 2. 定义响应式数据
// 给定一个默认结构，防止渲染报错
const properties = ref<EntryNodeProperties>();
...
onMounted(() => {
  ...
  // A. 初始化：首次加载时读取数据
  updateData(node.properties as EntryNodeProperties);
  ...
  graph.eventCenter.on(
    ...
    (eventData: { id: string; properties: unknown }) => {
      ...
      updateData(eventData.properties as EntryNodeProperties);
    },
  );
});
</script>
